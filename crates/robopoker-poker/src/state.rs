...
//! Fields are ordered largest-to-smallest for optimal alignment (AC-1.5).
//!
...
//! | Table          |        1,136 | Header (176) + 10 seats (960)   |
...
/// Betting street values (AC-5.1: betting rounds)
pub mod street {
...
/// AC-3.1: CRISPS mint is Token-2022 with authority recorded in config.
/// AC-3.4: Rake percentage stored here for hand settlement.
#[repr(C)]
...
pub struct Config {
    ...
    pub _padding: [u8; 3],
    /// Rake percentage in basis points (AC-3.4)
...
pub struct Seat {
    ...
    pub player: Pubkey,
    /// Player's chip stack at this table (u64 per AC-1.3)
    pub stack: u64,
    /// Player's current bet in this street (AC-5.1: stake matching)
    pub current_bet: u64,
    /// Total amount player has contributed to pot this hand
    pub total_bet: u64,
    /// Hash of hole cards for this hand (AC-2.6: privacy hybrid)
...
/// AC-3.2: Each table has a PDA-owned vault token account for escrow.
/// AC-4.1: Tables support MAX_SEATS = 10.
/// AC-4.4: Action timeouts enforced via slot-based deadlines.
/// AC-5.1: Betting rounds enforce turn order and stake matching.
/// AC-2.6/2.7/2.8: Privacy hybrid with seed commitment for deck verification.
#[repr(C)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Table {
    /// Account discriminator
    ...
    /// Current betting street (AC-5.1: preflop=0, flop=1, turn=2, river=3)
    pub current_street: u8,
    /// Number of players still active in the hand (not folded)
    pub active_count: u8,
    /// Whether seed has been revealed for this hand (AC-2.7)
    ...
    /// Big blind amount
    pub big_blind: u64,
    /// Slot deadline for current action (AC-4.4: 0 = no deadline)
    pub action_deadline_slot: u64,
    /// Current bet amount to call this street (AC-5.1: stake matching)
    pub current_bet: u64,
    /// Minimum raise amount (AC-5.2: raise bounds)
    pub min_raise: u64,
    /// Total pot accumulated this hand
    pub pot: u64,
    /// Accumulated rake collected at this table (AC-3.4)
    /// Swept to staking pool rewards vault periodically
    pub rake_accumulated: u64,
    /// Vault token account address (PDA-owned, Token-2022)
    pub vault: Pubkey,
    /// Seed commitment hash for this hand (AC-2.7: sha256(seed))
    /// Provider commits to this before cards are dealt
    pub seed_commitment: [u8; 32],
    /// The revealed seed (zeroed until seed is revealed) (AC-2.7)
    pub revealed_seed: [u8; 32],
    /// Seats array (fixed-size for on-chain, AC-1.2)
...
impl Table {
    ...
    /// Load table from account data (zero-copy)
    ///
    /// # Safety
    /// Caller must ensure data length >= TABLE_SIZE
    #[inline]
    pub unsafe fn from_bytes_unchecked(data: &[u8]) -> &Self {
        unsafe { &*(data.as_ptr() as *const Self) }
    }

    /// Load mutable table from account data (zero-copy)
    ///
    /// # Safety
    /// Caller must ensure data length >= TABLE_SIZE
    #[inline]
    ...
    /// Count active players (not folded, not sitting out)
    #[inline]
    pub fn count_active(&self) -> u8 {
        self.seats.iter().filter(|s| s.is_active()).count() as u8
    }

    /// Count players who can still act (occupied, not all-in)
    #[inline]
    pub fn count_can_act(&self) -> u8 {
        self.seats.iter().filter(|s| s.can_act()).count() as u8
    }
...
pub mod table_status {
    ...
    /// Showdown - awaiting seed reveal (AC-2.7)
    pub const SHOWDOWN: u8 = 3;
}

/// Global staking pool account (PDA: [b"staking_pool"])
///
/// Manages the global stake pool for rake distribution (AC-3.4, AC-3.5, AC-3.6).
...
pub struct StakingPool {
    ...
    /// Padding for alignment
    pub _padding: [u8; 6],
    /// Total CRISPS staked by all stakers (AC-3.5)
    pub total_staked: u64,
    /// Accumulated rake rewards available for distribution (AC-3.4)
    pub accumulated_rewards: u64,
    /// Total rewards ever distributed (for accounting)
...
impl StakingPool {
    ...
    /// Load staking pool from account data (zero-copy)
    ///
    /// # Safety
    /// Caller must ensure data length >= STAKING_POOL_SIZE
    #[inline]
    pub unsafe fn from_bytes_unchecked(data: &[u8]) -> &Self {
        unsafe { &*(data.as_ptr() as *const Self) }
    }

    /// Load mutable staking pool from account data (zero-copy)
    ///
    /// # Safety
    /// Caller must ensure data length >= STAKING_POOL_SIZE
    #[inline]
...
///
/// Tracks a single staker's deposited CRISPS and reward claims (AC-3.5, AC-3.6).
#[repr(C)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct StakerPosition {
    ...
    /// The staker's pubkey
    pub staker: Pubkey,
    /// Amount of CRISPS staked by this staker (AC-3.5)
    pub staked_amount: u64,
    /// Rewards already claimed by this staker (AC-3.6)
    pub rewards_claimed: u64,
    /// Snapshot of accumulated_rewards / total_staked at last claim
    /// Used to calculate proportional rewards since last claim
...
impl StakerPosition {
    ...
    /// Load staker position from account data (zero-copy)
    ///
    /// # Safety
    /// Caller must ensure data length >= STAKER_POSITION_SIZE
    #[inline]
    pub unsafe fn from_bytes_unchecked(data: &[u8]) -> &Self {
        unsafe { &*(data.as_ptr() as *const Self) }
    }

    /// Load mutable staker position from account data (zero-copy)
    ///
    /// # Safety
    /// Caller must ensure data length >= STAKER_POSITION_SIZE
    #[inline]
